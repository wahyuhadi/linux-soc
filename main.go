...
func displaySockInfo(proto string, s []netstat.SockTabEntry) {
	...
	for _, e := range s {
		if e.State.String() == "ESTABLISHED" {
			...
			localaddress := strings.Split(saddr, ":")
			// check for localaddress like ssh
			if isPortWhiteList(localaddress[1]) {
				continue
			}
			pid := strings.Split(p, "/")
			if isShellAgent(pid[1]) {
				msg := fmt.Sprintf("[!] %s Reverse Shell in %s to %v interact with %s shell ", e.State, p, daddr, pid[1])
				fmt.Println(msg)
				continue
			}
			spid := pid[0]
			...
			for _, cid := range tree.Procs[ipid].Children {
				shell := tree.Procs[cid].Stat.Comm
				if isShellAgent(shell) {
					msg := fmt.Sprintf("[!] %s Reverse Shell in %s to %v interact with %s shell ", e.State, p, daddr, shell)
					fmt.Println(msg)
				}
			}
		}
...
func isShellAgent(isShel string) bool {
	switch isShel {
	case "sh":
		return true
	case "/bin/sh":
...
func isPortWhiteList(port string) bool {
	switch port {
	case "22":
		return true
	}
